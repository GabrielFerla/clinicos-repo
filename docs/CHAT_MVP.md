...
| **O modelo pula a ferramenta em ~10% dos turnos** (18/20 medido) e responde de memória — chegou a negar que a clínica tenha estacionamento. E `tool_choice="required"` **não resolve: o Ollama 0.30.10 aceita e ignora** (0/5 tanto com `required` quanto com `auto`, medido isoladamente). Reforçar por prompt piora (12/16 contra 14/16). | Fallback determinístico: quando o modelo não chama ferramenta, o **servidor** consulta o FAQ. Se houver resposta relevante, o turno é refeito em cima do dado real. Fica visível na auditoria como `buscar_faq:fallback`. |
