...
> **Como este documento se aplica ao ClinicOS** — leia esta caixa antes do resto.
>
> O texto abaixo é o guia original do design system, em inglês, como veio do bundle de design. Ele é a fonte da verdade sobre a **intenção visual**: a direção, o do/don't, quando usar cada acento. Mantivemos na íntegra de propósito — reescrever teria custado nuance.
>
> O que muda na adaptação para este repositório:
>
> | O guia diz | Aqui é |
> |---|---|
> | "Link the one stylesheet from every page" | Tokens e componentes estão em `apps/theme/static_src/src/styles.css`, compilados junto do Tailwind. O `base.html` já carrega tudo via `{% tailwind_css %}`. Não há folha separada para linkar. |
> | `components/*.html`, `foundations/*.html`, `templates/` | Não portados. As páginas de referência do bundle ficaram fora do repo. |
> | `print-plates.js` e o tratamento `.cmyk` | **Não portados.** O design entrega com as fotos em `.halftone`, que é CSS puro, e `.cmyk-head`/`.cmyk-num` também são CSS puro (`text-shadow` + `mix-blend-mode`). Ligar o `.cmyk` exigiria trazer os filtros SVG de separação para dentro do documento. |
> | `theme.json` | Não portado. Para retunar o visual, edite o `:root` do `styles.css` — é o único lugar. |
> | "Use Phosphor icons, duotone" | Sem pacote de ícones instalado. Os poucos em uso (menu, balão do chat) são SVG do Phosphor inline, copiados um a um. Não adicione CDN: a rede do cliente piloto bloqueia. |
> | Fallback de fonte `system-ui, sans-serif` | Trocado para `Georgia, "Times New Roman", serif`. O fallback original é uma **sans** — sem rede para o Google Fonts a página inteira trocaria de voz, num sistema em que o serif *é* o cromo. |
> | `text-box: trim-both cap alphabetic` nas chapas | Removido. Lá o elemento hospedeiro aparava a própria caixa e as chapas precisavam acompanhar; aqui o hospedeiro não apara, e aparar só as chapas as jogaria meia entrelinha abaixo do papel. |
>
> Utilitários que **acrescentamos** ao sistema, para os móveis de primeira página: `.regra-dupla` (o par grosso-fino), `.regra-fina`, `.sobrelinha` (o rótulo em versalete que abre cada seção) e `.conduto` (o condutor pontilhado do índice).
>
> Uma advertência do guia que vale repetir porque é requisito de acessibilidade (S4-19): **o par acento/fundo é 3:1** — serve para ícones, texto grande e cromo de interface, **não** para corpo de texto. Parágrafo em acento usa `--color-accent-700`.
